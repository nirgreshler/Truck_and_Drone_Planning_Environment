...
def generate_random_obstacles(X, start, end, n, nodes=None):
    """
    ...
    while i < n:
        ...
        for j in range(X.dimensions):
            ...
            max_edge_length = (X.dimension_lengths[j][1] - X.dimension_lengths[j][0]) / 20.0
            # None of the sides of a hyperrectangle can be higher than 0.01 of the total span
            # in that particular X.dimensions
            min_edge_length = (X.dimension_lengths[j][1] - X.dimension_lengths[j][0]) / 200.0
            ...
            if nodes is not None:
                for node in nodes:
                    if abs(node[j] - center[j]) <= edge_length:
                        ncollision = True
                        break
        ...
        if len(list(X.obs.intersection(obstacle))) > 0 or scollision or fcollision or ncollision:
            continue
        i += 1
...
def generate_random_obstaclesV2(X, X_outer, corners, n, nodes=None):
    """
    Generates n random obstacles without disrupting world connectivity.
    It also respects start and end points so that they don't lie inside of an obstacle.
    """
    ...
    while i < n:
        ...
        for j in range(X.dimensions):
            ...
            for corner in corners:
                if abs(corner[j] - center[j]) <= edge_length:
                    ccollision = True
                    break
        ...
        obstacle = np.append(min_corner, max_corner)
        # Check newly generated obstacle intersects any former ones. Also respect start and end points
        if len(list(X.obs.intersection(obstacle))) > 0 or ccollision or ncollision:
            continue
        i += 1
        obstacles.append(obstacle)
        X.obs.add(uuid.uuid4(), tuple(obstacle), tuple(obstacle))
        X_outer.obs.add(uuid.uuid4(), tuple(obstacle), tuple(obstacle))
