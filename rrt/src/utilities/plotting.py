...
class Plot(object):
    def __init__(self, filename, plot_size=1000, title='Plot'):
        """
        Create a plot
        :param filename: filename
        """
        os.makedirs('./rrt/output/visualizations/', exist_ok=True)
        self.filename = "./rrt/output/visualizations/" + filename + ".html"
    ...
    def plot_path(self, X, path, c="red"):
        """
    ...
    def plot_nodes(self, X, nodes, color, size):
        """
        Plot a node
        :param X: Search Space
        :param node: node coordinates
